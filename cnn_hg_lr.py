...
median_filter = 5  # 1 3 5 7 9 11 13 15 17
d_num = 232
j_num = 40
m_num = 128
h_num = 208
...
batch_size = 10
learning_rate_set = [0.001]   # [0.0001,0.001,0.002,0.003,0.004,0.005,0.006,0.007,0.008,0.009,0.01,0.02,0.03]
num_epochs = 500
...
for learning_rate in learning_rate_set:
    ...
    for epoch in range(num_epochs):
        ...
        txt = "result/prediction/cnn_hg_lr_{}.txt".format(int(learning_rate * 1000))
    ...
    # 保存模型参数
    torch.save(model.state_dict(), 'parameter/cnn_hg_lr_{}.pth'.format(int(learning_rate*1000)))

    # 训练集
    txt = "result/cnn_hg_lr_{}.txt".format(int(learning_rate*1000))
    np.savetxt(txt, np.column_stack((all_epoch_loss, all_epoch_r2, all_epoch_mae, all_epoch_rmse)), fmt="%.8f",
               comments='')
    # 测试集
    txt = "result/cnn_hg_test_lr_{}.txt".format(int(learning_rate * 1000))
    np.savetxt(txt, np.column_stack((all_test_epoch_loss, all_test_epoch_r2, all_test_epoch_mae, all_test_epoch_rmse)), fmt="%.8f",
               comments='')
...
txt = "result/prediction/cnn_hg_lr_{}.txt".format(int(learning_rate*1000))
np.savetxt(txt, np.column_stack((test_preds, test_labels)),
               fmt="%.8f", comments='')
