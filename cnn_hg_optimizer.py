...
optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9)  # Adam SGD Adagrad momentum
...
# 保存模型参数
torch.save(model.state_dict(), 'parameter/cnn_hg_optimizer_momentum.pth')

txt = "result/cnn_hg_optimizer_momentum.txt"
np.savetxt(txt, np.column_stack((all_epoch_loss, all_epoch_r2, all_epoch_mae, all_epoch_rmse)), fmt="%.8f", comments='')
...
txt = "result/cnn_hg_test_optimizer_momentum.txt"
np.savetxt(txt, np.column_stack((mse, r2, mae, rmse)), fmt="%.8f", comments='')
