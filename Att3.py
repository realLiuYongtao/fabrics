...
class GlobalAttention(nn.Module):
    ...
    def forward(self, x):
        attention = self.sigmoid(self.conv(x))  # (B, 1, L)
        return x * attention
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.global_attention = GlobalAttention(32)  # 添加全局注意力
    ...
    def forward(self, x):
        ...
        x = self.global_attention(x)  # 添加全局注意力
        x = self.conv5(x)  # 18*1
...
# 保存模型参数
torch.save(model.state_dict(), './Att3.pth')
